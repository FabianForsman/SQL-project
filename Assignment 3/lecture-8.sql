...
-- If the trigger takes place BEFORE, then the "bad" update will be allowed
DROP TRIGGER IF EXISTS minimumBankBalance ON Accounts;
...
CREATE TABLE Submissions (
  ...
  FOREIGN KEY (student, course) REFERENCES Registrations, -- 2)
  UNIQUE(student, assignment, stime));     -- 6)

CREATE TABLE SubmittedFileTables (
  submission INT,
  filename TEXT,
  contents TEXT,
  FOREIGN KEY (submission) REFERENCES Submissions ON DELETE CASCADE, -- 4)
  PRIMARY KEY (submission, filename));

CREATE VIEW SubmittedFile AS -- 3)
  SELECT *, length (contents) AS filesize
  FROM SubmittedFileTables;

-- Triggers need to be implemented! :)
-- 1) to make sure submission id increases with new submissions
-- 5) to delete a submission when all the files of a submission are deleted